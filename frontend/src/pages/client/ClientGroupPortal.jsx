...
export default function ClientGroupPortal() {
  ...
  const handleLocateTree = (tree) => {
    ...
    if (lat == null || lng == null) {
      ...
      if (obs) { lat = obs.latitude; lng = obs.longitude; timelineIdx = obs.timeline_index; }
    }
    if (lat == null || lng == null) return;
    setLocateTarget({ lat, lng, timelineIdx });
    setActiveView('map');
  };
  ...
  return (
    <div className="page-bg min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex flex-col relative z-0">
        {group.status === 'READY' ? (
          activeView === 'map'
            ? <GroupMapView group={group} locateTarget={locateTarget} />
            : <GroupAnalyticsView group={group} onLocateTree={handleLocateTree} />
        ) : (
