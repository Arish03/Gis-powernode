...
import {
  ArrowLeft, CheckCircle2, Loader2, Send, ShieldAlert, X,
} from 'lucide-react';
...
const SEVERITY_OPTIONS = [
  { value: 'S5', label: 'Category A', color: '#b91c1c' },
  { value: 'S4', label: 'Category B', color: '#ea580c' },
  { value: 'S3', label: 'Category C', color: '#d97706' },
  { value: 'S2', label: 'Kuo', color: '#65a30d' },
...
export default function PowerlineSummary() {
  ...
  const severityMap = useMemo(() => {
    const m = {};
    (summary?.severity_counts || []).forEach(s => { m[s.severity] = s.count; });
    return m;
  }, [summary]);
  ...
  const statusLabel = project?.status === 'READY' ? 'Published'
    : project?.status === 'REVIEW_PENDING' ? 'Pending Review'
  ...
  return (
    <div className="page-bg min-h-screen flex flex-col">
      ...
      <div className="relative z-10 max-w-screen-xl mx-auto w-full px-4 sm:px-6 mt-4 mb-4 flex items-center gap-3 flex-wrap">
        <button
          onClick={() => navigate(`/admin/projects/${projectId}/powerline/annotate`)}
        ...
        <span className={[
          ...
          project?.status === 'REVIEW_PENDING' ? 'bg-amber-100 text-amber-700' :
      ...
      {error && (
        <div className="relative z-10 max-w-screen-xl mx-auto w-full px-4 sm:px-6 mb-3">
          ...
          <GlassCard className="p-6 space-y-5">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Full name of the primary inspector"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                className="glass-input text-sm min-h-[180px] resize-y"
                value={reportSummary}
                onChange={e => setReportSummary(e.target.value)}
                placeholder="Describe the inspection: scope, key findings, cost estimates, recommendations…"
              />
            ...
            <div className="flex items-center gap-3 pt-2 border-t border-slate-100 flex-wrap">
              <button
                onClick={saveMeta}
                disabled={saving}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-100 text-slate-700 text-sm font-semibold hover:bg-slate-200 disabled:opacity-50"
              >
                Save Draft
              </button>
              ...
              {isAdmin ? (
                ...
                <button
                  ...
                  disabled={!reportSummary.trim() || !inspectorName.trim() || saving}
                  className="btn-primary gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              {isAdmin && !canPublish && (
                <p className="text-[11px] text-amber-600">
                  {!reportSummary.trim() || !inspectorName.trim()
                    ? 'Fill in summary and inspector name to publish.'
                    : 'At least one annotated image is required to publish.'}
                </p>
              )}
            </div>
          </GlassCard>
