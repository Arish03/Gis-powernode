  ...
  const fetch = async () => {
    try {
      const [gRes, cRes] = await Promise.all([
        api.get(`/groups?client_id=${clientId}`),
        api.get('/users/clients').catch(() => ({ data: [] })),
      ]);
    ...
    <div className="page-bg min-h-screen">
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            onClick={() => navigate(`/admin/groups/new?clientId=${clientId}`)}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {groups.map(g => (
              <GlassCard
                key={g.id}
                className="p-5 cursor-pointer hover:shadow-glass-hover transition"
                onClick={() => navigate(`/admin/groups/${g.id}/view`)}
              >
