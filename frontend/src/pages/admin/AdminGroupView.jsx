...
export default function AdminGroupView() {
  ...
  const handleRecompute = async () => {
    try {
      await api.post(`/groups/${groupId}/recompute`);
      fetchGroup();
      fetchStatus();
    } catch (e) {
      console.error(e);
  ...
  if (loading) return <SidebarLayout title="Admin Group View"><div className="spinner" /></SidebarLayout>;
  if (!group) return <SidebarLayout title="Admin Group View"><div className="p-10 text-center text-red-500">Group not found</div></SidebarLayout>;
  return <SidebarLayout title="Admin Group View">
      
      <header className="relative z-10 max-w-screen-2xl mx-auto w-full px-4 pt-4 pb-2 flex items-center gap-3">
        <button onClick={() => navigate(group.client_id ? `/admin/clients/${group.client_id}/groups` : '/admin')} className="p-2 rounded-lg hover:bg-white/60">
          <ArrowLeft size={18} />
        </button>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleRecompute} title="Recompute matching" className="px-3 py-1.5 rounded-lg text-xs font-semibold inline-flex items-center gap-1.5 bg-white/70 text-slate-600 hover:bg-white">
            <RefreshCcw size={14} /> Recompute
          </button>
        </div>
      </header>

      {group.status === 'PROCESSING' && <div className="max-w-screen-2xl mx-auto w-full px-4">
          <GlassCard className="p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="mt-2 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                <div className="h-full bg-primary-500 transition-all" style={{
      ...
      {group.status === 'ERROR' && <div className="max-w-screen-2xl mx-auto w-full px-4">
      ...
      <main className="flex-1 flex flex-col relative z-0">
        {group.status === 'READY' ? activeView === 'map' ? <GroupMapView group={group} /> : <GroupAnalyticsView group={group} /> : group.status === 'PENDING' ? <div className="flex-1 flex items-center justify-center text-slate-500">Group pending — waiting for matcher to start.</div> : <div className="flex-1" />}
      </main>
    </SidebarLayout>;
}