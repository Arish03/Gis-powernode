...
import {
  ...
  Layers, Scan, BarChart2, Cpu, AlertTriangle, RefreshCcw, Trees, Zap,
} from 'lucide-react';
  ...
  const handleSaveDetails = async (e) => {
    ...
    try {
      if (isEditing) {
        ...
        const { project_type: _pt, ...editable } = formData;
        await api.put(`/projects/${id}`, editable);
        setCurrentStep(2);
      } else {
        const res = await api.post('/projects', formData);
        if (formData.project_type === 'POWERLINE') {
          navigate(`/admin/projects/${res.data.id}/powerline/annotate`, { replace: true });
          return;
        }
            ...
            <form onSubmit={handleSaveDetails} className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { value: 'TREE', label: 'Plantation / Trees', sub: 'Drone photogrammetry + AI tree detection', Icon: Trees, color: 'text-emerald-600', ring: 'ring-emerald-500', bg: 'bg-emerald-50' },
                    { value: 'POWERLINE', label: 'Power Transmission Line', sub: 'Manual image annotation + PDF report', Icon: Zap, color: 'text-amber-600', ring: 'ring-amber-500', bg: 'bg-amber-50' },
                  ].map(opt => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        disabled={disabled}
                        onClick={() => !disabled && setFormData({ ...formData, project_type: opt.value })}
                        className={[
                          'flex items-start gap-3 p-3 rounded-xl border text-left transition-all',
                          selected
                            ? `${opt.bg} border-transparent ring-2 ${opt.ring} shadow-sm`
                ...
                {isEditing && (
                  <p className="text-[10px] text-slate-400 mt-1.5">Project type cannot be changed after creation.</p>
                )}
              </div>
                  ...
                  {selectedFiles.length >= 2 && (
                    <button onClick={handleStageImages}
                      ...
                      {uploadPhase === 'uploading'
                        ...
                        : <><UploadCloud size={13} /> Stage {selectedFiles.length} Images</>}
                    </button>
                  )}
                  {stagedCount >= 2 && !isSubAdmin && (
                    <div className="flex items-center gap-2">
                      {processingNodes.length > 0 && (
                        <select
                          value={selectedNodeId}
                          onChange={e => setSelectedNodeId(e.target.value)}
                          className="glass-input text-xs py-1.5 px-2 pr-7 min-w-[160px]"
                        >
                          <option value="auto">Auto (least busy)</option>
                          {processingNodes.map(n => (
